---
date: '2025-07-21'
description: A critical security advisory has been issued for a deprecated Anthropic
  Slack MCP Server, which is susceptible to data leakage through a "link unfurling"
  vulnerability. This flaw allows an AI agent to leak sensitive information when links
  are posted in Slack, exposing internal data to external servers via prompt injection
  attacks. The MCP Server is widely used, with over 14,000 weekly downloads, raising
  significant security concerns. Since Anthropic has ceased maintenance and patching,
  users are advised to disable link unfurling in their configurations and reassess
  their applications' security posture to mitigate risks.
