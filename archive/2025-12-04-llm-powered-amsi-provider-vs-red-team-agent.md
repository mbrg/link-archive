---
date: '2025-12-04'
description: The paper discusses the development of a Low-Level Mechanism for an LLM-powered
  AMSI (AntiMalware Scan Interface) provider that enhances PowerShell script analysis
  to distinguish between benign and malicious scripts in real-time execution contexts.
  By integrating a red team agent designed to execute scripts with malicious intent,
  the study generates comprehensive datasets that reveal the efficacy of the AMSI
  provider in identifying privilege escalation attempts and other security threats.
  The results show that leveraging LLMs allows for adaptive defenses that evolve in
  response to offensive tactics, creating a feedback loop that enhances overall cybersecurity
  efficacy.
