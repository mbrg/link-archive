---
date: '2025-12-21'
description: '**Performance Hints** by Jeff Dean and Sanjay Ghemawat offers a comprehensive
  guide on performance tuning in software development. Key insights include strategies
  for code efficiency, such as favoring faster alternatives that maintain code readability,
  and leveraging algorithms with lower time complexities. The document emphasizes
  the importance of profiling and measurement to identify bottlenecks, as well as
  applying bulk operations to enhance performance. Recommendations include use of
  cache-friendly data structures like `absl::flat_hash_map` and `gtl::small_map`,
  reducing allocations, and avoiding unnecessary logging in critical sections. The
  guide serves as a vital resource for optimizing software performance.'
...
Expand all detailsCollapse all details
...
_Many of the examples in the document have code fragments that demonstrate the_
_techniques (click the little triangles!)._ _Note that some of these_
_code fragments mention various internal Google codebase abstractions. We have_
_included these anyway if we felt like the examples were self-contained enough to_
_be understandable to those unfamiliar with the details of those abstractions._

## The importance of thinking about performance

Knuth is often quoted out of context as saying _premature optimization is the_
_root of all evil_. The
[full quote](https://dl.acm.org/doi/pdf/10.1145/356635.356640) reads: _“We_
_should forget about small efficiencies, say about 97% of the time: premature_
_optimization is the root of all evil. Yet we should not pass up our_
_opportunities in that critical 3%.”_ This document is about that critical
...
results together.
3. The preceding gives the _cost_ of the system in terms of resource usage. If
...
have [pitfalls](https://abseil.io/fast/39) that make them non-representative of full system
performance. Useful libraries for writing microbenchmarks:
[C++](https://github.com/google/benchmark/blob/main/README.md) [Go](https://pkg.go.dev/testing#hdr-Benchmarks) [Java](https://github.com/openjdk/jmh).
...
[changes that demonstrate multiple techniques](https://abseil.io/fast/hints.html#cls-that-demonstrate-multiple-techniques)
section.
...
listed under [algorithmic improvements](https://abseil.io/fast/hints.html#algorithmic-improvements) can be
...
[get an allocation profile](https://gperftools.github.io/gperftools/heapprofile.html), and pick away at the highest
...
[profiling tools and tips](https://abseil.io/fast/hints.html#profiling-tools-and-tips) section can be
helpful.
...
Add RPC\_Stats::RecordRPC variant allowing client to pass in
...
watch out for [cache-line contention](https://abseil.io/fast/hints.html#reduce-false-sharing).\
\
### Memory layout\
\
...
\
Use an array instead of flat\_map.\
\
rtp\_controller.h\
\
```c++\
...
dense\_hash\_set<ZoneId> with a bit-vector with one bit per zone.\
\
zone\_set.h\
\
```c++\
...
\
Pre-size a vector and fill it in, rather than N push\_back\
operations.\
\
...
```\
\
Use std::sort instead of std::stable\_sort, which avoids\
...
```\
\
Skip significant work in RPC\_Stats\_Measurement addition if\
...
helpful [display of source code interleaved with disassembly](https://github.com/google/pprof/blob/main/doc/README.md#annotated-source-code)\
...
Hand unroll\
[cyclic\\
redundancy check](https://en.wikipedia.org/wiki/Cyclic_redundancy_check) (CRC) computation loop.\
\
crc.cc\
\
```c++\
...
```\
\
Avoid frame setup costs by converting ABSL\_LOG(FATAL) to\
ABSL\_DCHECK(false).\
\
arena\_cleanup.h\
\
...
```\
\
Shrink each RETURN\_IF\_ERROR call site by 79 bytes of\
code.\
\
1. Added special adaptor class for use by just RETURN\_IF\_ERROR.\
2. Do not construct/destruct StatusBuilder on fast path of RETURN\_IF\_ERROR.\
3. Do not inline some StatusBuilder methods since they are now no longer needed\
on the fast path.\
4. Avoid unnecessary ~Status call.\
\
Improve performance of CHECK\_GE by 4.5X and shrink code\
...
```\
\
Reduce absl::flat\_hash\_set and absl::flat\_hash\_map code\
size.\
\
1. Extract code that does not depend on the specific hash table type into\
common (non-inlined) functions.\
2. Place ABSL\_ATTRIBUTE\_NOINLINE directives judiciously.\
...
```\
\
Reduce generated code size for absl::flat\_hash\_set and\
absl::flat\_hash\_map.\
\
- Extract code that does not depend on the specific hash table type into\
common (non-inlined) functions.\
- Place ABSL\_ATTRIBUTE\_NOINLINE directives judiciously.\
...
`absl::flat_hash_map` discussion below in [Bulk Operations](https://abseil.io/fast/hints.html#bulk-operations)\
section).\
\
...
can be [dangerous](https://abseil.io/docs/cpp/atomic_danger). Prefer higher-level abstractions.\
\
...
`std::unordered_map`.\
\
Speed up LanguageFromCode (use absl::flat\_hash\_map\
instead of a \_\_gnu\_cxx::hash\_map).\
\
languages.cc\
\
```c++\
...
uses dense\_hash\_map instead of absl::flat\_hash\_map, which did not exist at the\
time).\
\
publish.cc\
\
...
```\
\
Use dense\_hash\_map instead of hash\_map for keeping track of\
SelectServer alarms (would use absl::flat\_hash\_map today).\
\
alarmer.h\
\
```c++\
...
\
absl::btree\_map and absl::btree\_set store multiple entries per tree node. This\
...
\
Use btree\_set instead of std::set to represent a very heavily used\
work-queue.\
\
...
### gtl::small\_map\
\
gtl::small\_map uses an inline array to store up to a certain number of unique\
...
\
Use gtl::small\_map in tflite\_model.\
\
tflite\_model.cc\
\
```c++\
...
\
gtl::small\_ordered\_set is an optimization for associative containers (such as\
std::set or absl::btree\_multiset). It uses a fixed array to store a certain\
...
\
Use gtl::small\_ordered\_set to hold set of listeners.\
\
broadcast\_stream.h\
\
```c++\
...
\
Use intrusive\_list to keep track of inflight requests for\
...
```\
\
In TF\_CHECK\_OK, avoid creating Ok object in order to test\
for ok().\
\
status.h\
\
...
\
absl::flat\_hash\_map compares one hash byte per key from a\
...
\
3. Rather than having the bins\_ data structure be a std::set and using\
lower\_bound to locate the appropriate bin given a byte\_size, we instead have\
an array of bins, indexed by a function that is log₂(byte\_size/256). This\
...
memory without involving the retry\_helper\_. If an initial attempt fails\
(returns nullptr), then we go through the retry\_helper\_, but normally we can\
...
\
Speeds up ptb\_word\_lm on my desktop machine with a Titan X card from 8036 words\
...
\
- DescriptorProto.inlined\_contents is now a string, not a Cord (it is expected\
to be used only for small-ish tensors). This necessitated the addition of a\
bunch of copying helpers in tensor\_util.cc (need to now support both strings\
and Cords).\
\
- Use flat\_hash\_map instead of std::unordered\_map in a few places.\
\
...
\
   - Pre-form a ShapeUtil::ForEachState::indexes\_span pointing at the\
     ShapeUtil::ForEachState::indexes vector, rather than constructing this\
     span from the vector on every loop iteration.\
\
   - Save a ShapeUtil::ForEachState::indexes\_ptr pointer to the backing store\
...
\
   - Save a ShapeUtil::ForEachState::minor\_to\_major array pointer initialized\
     in the constructor by calling shape.layout().minor\_to\_major().data()\
...
3. Improved performance of LiteralBase::Broadcast in several ways:\
   - Introduced templated BroadcastHelper routine in literal.cc that is\
     ...
     primitive\_size was a runtime variable and so the compiler couldn't do a\
     ...
     1, 2, 4, or 8)).\
\
   - Avoided all but one of ~(5 + num\_dimensions + num\_result\_elements)\
     ...
     down to "root\_piece().subshape()", where subshape() is a virtual\
     function.\
\
...
\
   - In BroadcastHelper, used a scratch\_source\_array pointer variable that\
     points into the backing store of the scratch\_source\_index vector, and\
     used that directly to avoid vector::operator\[\] operations inside the\
     per-element code. Also pre-computed a scratch\_source\_span that points to\
     the scratch\_source\_index vector outside the per-element loop in\
...
\
   - Introduced new three-argument variant of\
     ...
     the minor\_to\_major span associated with the shape argument. Used this in\
...
is done by BM\_ForEachIndexNoStatus/NUM is comparable to the BM\_ForEachIndex/NUM\
results above).\
\
```{.bench}\
Benchmark                                      Base (ns)    New (ns) Improvement\
...
\
   - It significantly reduces memory usage, since we only use\
     "sizeof(ValuePtr) + value\_len" bytes for each value, rather than\
     "sizeof(SafeCombinerKey) + sizeof(StringPiece) + value\_len + new hash\
...
\
2. Converted AlarmList in Alarmer from a hash\_map to a dense\_hash\_map to avoid\
...
13.1 MB/s for a microbenchmark).\
...
multiplying by num\_shards\_. Only when we need the actual docid value do we\
multiply by num\_shards\_ and add my\_shard\_.\
...
- We only initialize the first nterms\_ elements of various scoring data\
structures, rather than initializing all MAX\_TERMS of them (in some cases,\
we were unnecessarily memsetting 20K to 100K of data per document scored).\
- Avoid round\_to\_int and subsequent computation on intermediate scoring values\
